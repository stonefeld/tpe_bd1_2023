...
CREATE TRIGGER valid_timestamp BEFORE INSERT OR UPDATE ON EMPLEADO_TT FOR EACH ROW EXECUTE PROCEDURE valid_timestamp();
...
CREATE TRIGGER call_action AFTER INSERT OR UPDATE OR DELETE ON EMPLEADO FOR EACH ROW EXECUTE PROCEDURE call_action();

-- Función para crear el historial de empleados a partir de la tabla empleado_tt
CREATE OR REPLACE FUNCTION historial_empleado(datetime IN timestamp without time zone) RETURNS VARCHAR AS $ $
DECLARE
    ret text;
    var_aux record;
BEGIN
...
-------------------------------------------------------';
    ...
    FOR var_aux IN
        SELECT CASE
		    ...
		    ELSE 'NO VIGENTE       '
	    END estado, legajo, sueldo, edad, tt_izq,
	    (SELECT COUNT(*) FROM EMPLEADO_TT AS tt
        WHERE tt.tt_izq <= my_t.tt_izq
        AND tt.legajo = my_t.legajo) AS nro_mov FROM EMPLEADO_TT AS my_t
        GROUP BY legajo, tt_izq
        ORDER BY legajo
    LOOP
